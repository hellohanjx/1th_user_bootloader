...
* 返      回      : 返回长度.
*******************************************************************************/
uint8_t	CH376ReadBlock( puint8_t buf )
{
	uint8_t	s, l;
...
* 输      入      : puint8_t buf:
*                   指向发送缓冲区.
* 返      回      : uint8_t s：后续数据长度.
*******************************************************************************/
uint8_t	CH376WriteReqBlock( puint8_t buf )
{
	uint8_t	s, l;
...
* 返      回      : 无.
*******************************************************************************/
void	CH376WriteHostBlock( puint8_t buf, uint8_t len )
{
...
* 返      回      : 无.
*******************************************************************************/
void	CH376WriteOfsBlock( puint8_t buf, uint8_t ofs, uint8_t len )
{
...
* 返      回      : 无.
*******************************************************************************/
void	CH376SetFileName( puint8_t name )
{
	uint8_t	c;
...
* 返      回      : 无.
*******************************************************************************/
void	CH376WriteVar8( uint8_t var, uint8_t dat )
{
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376FileOpen( puint8_t name ) 
{
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376FileCreate( puint8_t name )
{
	if ( name ) 
	{
		CH376SetFileName( name );  	/* 设置将要操作的文件的文件名 */
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376DirCreate( puint8_t name )
{
	CH376SetFileName( name );  	/* 设置将要操作的文件的文件名 */
#ifndef	DEF_IC_V43_U
	if ( name[0] == DEF_SEPAR_CHAR1 || name[0] == DEF_SEPAR_CHAR2 ) 
...
*******************************************************************************/
uint8_t	CH376SeparatePath( puint8_t path )
{
	puint8_t	pName;
...
* 返      回      : 返回最后一级文件名或者目录名的字节偏移.
*******************************************************************************/
uint8_t	CH376FileOpenDir( puint8_t PathName, uint8_t StopName )
{
	uint8_t	i, s;
...
* 返      回      : 返回最后一级文件名或者目录名的字节偏移.
*******************************************************************************/
uint8_t	CH376FileOpenPath( puint8_t PathName )
{
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376FileCreatePath( puint8_t PathName )
{
	uint8_t	s;
	uint8_t	Name;
...
*					度不超过255个字符
* 输      入      : puint8_t path:	指向路径缓冲区.
* 返      回      : 操作状态.
*******************************************************************************/
#ifdef	EN_DIR_CREATE
uint8_t	CH376DirCreatePath( puint8_t PathName )
{
	uint8_t	s;
	uint8_t	Name;
	uint8_t	ClustBuf[4];
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376FileErase( puint8_t PathName )
{
	uint8_t	s;
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376ByteRead( puint8_t buf, uint16_t ReqCount, puint16_t RealCount )
{
	uint8_t	s;
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376ByteWrite( puint8_t buf, uint16_t ReqCount, puint16_t RealCount )
{
	uint8_t	s;
...
* 描      述      : 查询磁盘物理容量,扇区数.
* 输      入      : puint32_t DiskCap:
*                   磁盘容量.
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376DiskCapacity( puint32_t DiskCap )
{
	uint8_t	s;
...
* 描      述      : 查询磁盘剩余空间信息,扇区数.
* 输      入      : puint32_t DiskFre:
*                   当前逻辑盘的剩余扇区数.
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t   CH376DiskQuery( puint32_t DiskFre )
{
	uint8_t	s;
	uint8_t	c0, c1, c2, c3;
...
*                   uint8_t iSectorCount:
*                   iSectorCount 是准备读取的扇区数                   
* 返      回      : 中断状态.
*******************************************************************************/
#ifdef	EN_SECTOR_ACCESS
uint8_t	CH376DiskReadSec( puint8_t buf, uint32_t iLbaStart, uint8_t iSectorCount )
{
	uint8_t	s, err;
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376DiskWriteSec( puint8_t buf, uint32_t iLbaStart, uint8_t iSectorCount )
{
	uint8_t	s, err;
	uint16_t	mBlockCount;
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376SecRead( puint8_t buf, uint8_t ReqCount, puint8_t RealCount )
{
	uint8_t	s;
	uint8_t	cnt;
	uint32_t	StaSec;
	...
	do 
	{
		...
		if ( cnt == 0x41 ) 
		{
			...
			xReadCH376Data( );
			fofs = xReadCH376Data( );
			fofs |= (uint16_t)(xReadCH376Data( )) << 8;
			fofs |= (uint32_t)(xReadCH376Data( )) << 16;
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376SecWrite( puint8_t buf, uint8_t ReqCount, puint8_t RealCount )
{
	uint8_t	s;
	uint8_t	cnt;
	uint32_t	StaSec;
...
*******************************************************************************/
#ifdef	EN_LONG_NAME
uint8_t	CH376LongNameWrite( puint8_t buf, uint16_t ReqCount )
{
	uint8_t	s;
#ifndef	DEF_IC_V43_U
	uint8_t	c;
...
*                   节格式.
* 输      入      : puint8_t DirName:
*                   指向外部文件名缓冲区.
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376CheckNameSum( puint8_t DirName )
{
	uint8_t	NameLen;
	uint8_t	CheckSum;
...
* 输      入      : puint8_t PathName:
*                   指向路径缓冲区.
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376LocateInUpDir( puint8_t PathName ) 
{
	uint8_t	s;
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376GetLongName( puint8_t PathName, puint8_t LongName ) 
{
	uint8_t	s;
	uint16_t	NameCount;	                                                                /* 长文件名字节计数 */
	...
	while ( 1 ) 
	{
		...
		{  
			if ( s >= sizeof( FAT_DIR_INFO ) ) *(puint16_t)( &LongName[ NameCount ] ) = 0x0000;  /* 尚未收集到长文件名的结束符,则强制结束 */
...
* 返      回      : 中断状态.
*******************************************************************************/
uint8_t	CH376CreateLongName( puint8_t PathName, puint8_t LongName )
{
	uint8_t	s, i;
	uint8_t	DirBlockCnt;	                                                            /* 长文件名占用文件目录结构的个数 */
	uint16_t	count;			                                                            /* 临时变量,用于计数,用于字节读文件方式下实际读取的字节数 */
	uint16_t	NameCount;		                                                            /* 长文件名字节计数 */
	uint32_t	NewFileLoc;		                                                            /* 当前文件目录信息在上级目录中的起始位置,偏移地址 */
	
	for ( count = 0; count < LONG_NAME_BUF_LEN; count += 2 ) 
	{
	    if ( *(puint16_t)(&LongName[count]) == 0 ) 
	    {
	        break;                                                                      /* 到结束位置 */
	    }
	}
