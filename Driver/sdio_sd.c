...
{
  volatile u32 cnt = 7200000; /* ~100mS */
...
/**
  ...
  * @retval None
  */
void SDIO_IRQnSet(FunctionalState status)
{
...
/**
  * @brief  打印SD信息(2G/4G测试过,4G以上容量显示有误,待解决)
  * @param  None
  * @retval None
  */
void SD_Info(void)
{
	...
	uint16_t i;
	float Capacity=0.00;
	Capacity=(SDCardInfo.CardCapacity)/1024.00/1024.00/1024.00;
  printf("\r\nsd Capacity:%.2fG\r\n",Capacity);
  printf("\r\nsd BlockSize:%d\r\n",SDCardInfo.CardBlockSize);
  printf("\r\nsd Type:%d\r\n",SDCardInfo.CardType);             //type=1,小于等于2G;type=2,2G~32G
  printf("\r\n第一扇区数据最后两字节为 55 aa表读正确\r\n");
	SD_ReadBlock(readbuf, 0, 512);
	for(i=0;i<512;i++)
	{
		if( (i>=15) && (i%16 == 0))
		{
